...
export default async function Product({params}:{params:Promise<{slug:string}>}){
  ...
  const {data:p,error}=await s.from('products').select('*,brand:brands(name),category:categories(name)').eq('slug',slug).maybeSingle();
  ...
  return <main><header className="top"><Link href="/" className="brand">TIMEA <span>WATCH BOUTIQUE</span></Link>
    ...
    <section className="detail"><img src={p.image} alt={p.name}/><div><small>{p.brand?.name} · {p.category?.name}</small>
